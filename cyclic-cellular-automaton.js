var availableColors = ["#e0ffff","#90ee90","#d3d3d3","#ffb6c1","#9932cc","#8b0000", "#e9967a","#9400d3","#0000ff","#00ffff","#f5f5dc","#a52a2a","#00008b","#000000","#008b8b","#a9a9a9","#f0ffff","#006400","#bdb76b","#8b008b","#556b2f","#ff8c00","#ff00ff","#ffd700","#008000","#4b0082","#f0e68c","#add8e6","#ffffe0","#00ff00","#ff00ff","#800000","#000080","#808000","#ffa500","#ffc0cb","#800080","#800080","#ff0000","#c0c0c0","#ffffff","#ffff00"];

function CCAGenerateCanvas(width, height) {
	...
	for (let i = 0; i < width; ++i) {
		...
		for (let j = 0; j < height; ++j) {
			...
			fillPixel(ctx,randomRGBColorAndPosition[1],i,j);
			// console.log("x="+i+" / y="+j);
		}
	}
...
function CCAStart(width, height, maxIterations=20){
	let startConditions = CCAGenerateCanvas(width,height);
	// console.log("START CONDITIONS");
	// console.log(startConditions);
	CCALoop(startConditions[0], startConditions[1], maxIterations, i=0);
}

// LOOP
function CCALoop(matrix, availableRGBColors, maxIterations, i){
	let maxColor = availableRGBColors.length - 1;
	matrix = CCAChangeMatrixColors(matrix, maxColor);
	CCAChangeCanvasColorsFromMatrix(matrix, availableRGBColors);
	timeoutCCA = setTimeout(function() {
		CCALoop(matrix, availableRGBColors, maxIterations, i)
	}, 1);
	// console.log("##########")
...
function CCAChangeCanvasColorsFromMatrix(matrix, availableRGBColors) {
